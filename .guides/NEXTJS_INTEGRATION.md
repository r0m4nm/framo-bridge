...
### 4. Connect User (framo.app → Blender)

```
POST http://localhost:8080/connect-user
```

**Purpose:** Send user information from framo.app to Blender addon to display who is currently connected.
...
-   When user logs into framo.app
-   On framo.app page load (to restore connection state if user already authenticated)
...
```javascript
// When user logs into framo.app
async function notifyBlenderOfLogin(user) {
    try {
        const response = await fetch("http://localhost:8080/connect-user", {
            method: "POST",
        ...
        if (response.ok) {
            ...
            console.log("✓ Blender notified:", result.message);
        }
    } catch (error) {
        // Blender addon not running - this is fine
        console.log("Blender addon not available");
    }
}

// On app initialization (restore connection)
useEffect(() => {
    const currentUser = getCurrentUser(); // Your auth function
    if (currentUser) {
        notifyBlenderOfLogin(currentUser);
    }
}, []);
```
...
### 5. Disconnect User (framo.app → Blender)

```
POST http://localhost:8080/disconnect-user
```

**Purpose:** Clear user information in Blender when user logs out or closes framo.app.
...
-   `200` - User disconnected successfully

**When to call:**

-   When user logs out of framo.app
-   When framo.app window/tab is closed (use `beforeunload` event)
...
```javascript
// On logout
async function notifyBlenderOfLogout() {
    try {
        await fetch("http://localhost:8080/disconnect-user", {
            method: "POST",
        });
        console.log("✓ Blender notified of logout");
    } catch (error) {
        // Addon not running - ignore
    }
}
...
window.addEventListener("beforeunload", () => {
    ...
    navigator.sendBeacon("http://localhost:8080/disconnect-user", blob);
});
```
...
**Purpose:** Retrieve currently connected user information.
...
```typescript
{
  ...
  last_connected: string | null; // ISO 8601 timestamp
}
```

**Status Codes:**

-   `200` - Always returns 200 (returns null values if no user connected)
...
if (response.ok) {
    ...
    if (userInfo.name) {
        console.log(`Blender shows user: ${userInfo.name}`);
        console.log(`Last connected: ${userInfo.last_connected}`);
    } else {
        console.log("No user currently shown in Blender");
    }
}
```
...
### 7. Upload Model (Internal - used by Blender)
...
-   **Mesh Decimation** (optional, face reduction ratio 0.0-1.0, uses Blender's native BMesh)
...
The addon supports **two-way communication** between Blender and framo.app:

### Direction 1: Blender → framo.app (Model Export)
```
Blender user clicks "Send to Framo"
    ↓
Addon exports GLB with optimizations
    ↓
POST to /upload-model (stores in server memory)
    ↓
framo.app polls /latest-model (every 2s)
    ↓
framo.app receives and displays model
```

### Direction 2: framo.app → Blender (User Info)
```
User logs into framo.app
    ↓
framo.app sends POST to /connect-user
    ↓
Addon stores user info globally
    ↓
Blender UI updates to show "Connected User: John Doe"
    ↓
User in Blender sees who is receiving their models
```

### Visual Representation in Blender

When user connects from framo.app, the Blender addon UI shows:
...
This provides visual confirmation of:
- The server is running
- Someone is connected to receive models
- Who specifically is connected
...
-   Only stores the **latest** connected user (one user at a time)
...
-   User info is stored in memory only (cleared when addon restarts)
